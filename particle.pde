 ...
 // }
  void update() {
    ...
    //if (this.follow_mode) {
      ...
      acc.setMag(.01);
  //  }
    pos.x = map((float) noise.eval(xoff, yoff),0,1, 0, width);
    pos.y = map((float) noise.eval(yoff, xoff), 0, 1, 0, height);
    xoff += .003;//. 007
    yoff += .003;
  //  if (this.follow_mode) {
      vel.add(acc);
  ...
  //  print(history.length);
    ...
    for (int i = 0; i < history.size() ;i++) {
      ...
      float col1 = map(noise(this.xoff),0,1, 0, 100);
      float col2 = map(noise(this.xoff),0,1, 0, 100);
      noStroke();
      colorMode(RGB);
      if (this.ranCol < 1) {
        fill(col, 0, 0, 100);
